---
title: "WTC11 Unfractionated HCD Analysis"
...
R_experiment_name <- "R_WTC11_Unfrac" 
...
tom_result_1 <- read_csv("231205_AC11_unfract_MTproWTC11shTMTproAS192_1ug_Run1_Result.csv")
tom_result_2 <- read_csv("231205_AC11_unfract_MTproWTC11shTMTproAS192_1ug_Run2_Result.csv")
tom_result_3 <- read_csv("231205_AC11_unfract_MTproWTC11shTMTproAS192_1ug_Run3_Result.csv")
...
#tom_tmt_1 <- read_csv()
tom_tmt_2 <- read_csv("WTC11unfrac_MS1_MS2_Tomahto_Rep2_TMTpro_SkylineReport.csv")
tom_tmt_3 <- read_csv("WTC11unfrac_MS1_MS2_Tomahto_Rep3_TMTpro_SkylineReport.csv")

########################################Tomahto shTMT Skyline#################################

tom_sh_1 <- read_csv("WTC11unfrac_MS1_MS2_Tomahto_Rep1_shTMTpro_SkylineReport.csv")
tom_sh_2 <- read_csv("WTC11unfrac_MS1_MS2_Tomahto_Rep2_shTMTpro_SkylineReport.csv")
tom_sh_3 <- read_csv("WTC11unfrac_MS1_MS2_Tomahto_Rep3_shTMTpro_SkylineReport.csv")
...
tom_tmt <- bind_rows(tom_tmt_2, tom_tmt_3)
...
################################## Read in WTC11 CID File######################
cid_combined <- read_csv("240305_R_WTC11_Unfrac_CID_COMBINED_FOR_FIGURE.csv")
cid_combined <- cid_combined %>% 
  mutate(fragmentation = "NA") %>% 
  mutate(fragmentation = ifelse(source == "Tomahto", "CID", 
                                ifelse(source == "DDA", "HCD", fragmentation)))


##################Write tom csv###########################
write_csv(tom, "240320_unfrac_tom_results.csv")
```
...
write_csv(combined_for_fig, "240313_Tomahto_HCD_Rep1.csv")
```
...
### Generating Figures- Combined CID and HCD

```{r}
combined_for_fig <- combined_for_fig %>% 
  mutate(fragmentation = "HCD")

combined_hcd_cid <- bind_rows(combined_for_fig, cid_combined)
combined_hcd_cid <- combined_hcd_cid %>% 
  mutate(source_fragmenation = paste(source, fragmentation, sep = " ")) %>% 
  mutate(frac_status = "unfractionated")

plot_as192_bar_maj_min_cid_hcd <- combined_hcd_cid %>%
  group_by(Peptide, source_fragmenation) %>% 
  arrange(desc(total_area_MS1)) %>% 
  slice(1) %>%
  ungroup() %>% 
  ggplot(aes(x = source_fragmenation, fill = is_major_gene_isoform_char)) +
  geom_bar(position = "stack") +
    geom_text(stat = "count", aes(label = after_stat(count)), position = position_stack(vjust = 0.5)) +
  scale_fill_manual(values = c("major" = "#009988", "minor" = "#EE7733")) +
  labs(title = "AS192 Peptide Detection",
...
plot_as192_bar_maj_min_cid_hcd
```

### Export Combined hcd_cid
...
combined_hcd_cid %>% 
  ...
  slice(1) %>%
  ungroup() %>% 
  write_csv("240310_combined_unfrac_hcd_cid.csv")

write_csv(combined_hcd_cid, "240313_Tomahto_HCD_1.csv")
```
